...
@pytest.fixture
def sample_image():
    """400x400 BGR image with one gold-colored coin (circle) at center."""
...
def test_crop_roi_shape(segmenter, sample_image):
    ...
    # Should be approximately 2*(80 + 4) + 1 = 169x169
    pad = int(80 * 0.05)
    expected_size = 2 * (80 + pad)
    # Note: slice is [200-84 : 200+84] which is 168 pixels usually
...
# TC-3.3: Mask covers correct area (approximately pi*r^2)
def test_mask_area(segmenter, sample_image):
    roi, (x1, y1) = segmenter.crop_roi(sample_image, 200, 200, 80)
    cx_local = 200 - x1
    cy_local = 200 - y1
    # erosion is 2, so actual radius in mask is 78
    _, mask = segmenter.apply_circular_mask(roi, cx_local, cy_local, 80)
...
# TC-3.5: Normalized ROI is exactly target_size x target_size
...
def test_grid_image_dimensions(segmenter):
    t = 128
    # 3 coins -> 1 row of 8 columns -> 1 row x 8 cols = 128 x 1024
